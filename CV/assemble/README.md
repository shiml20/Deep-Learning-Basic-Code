> 史明磊 2020011245 自动化系
>
> 为避免重复造轮子，本代码启发自Pytorch官方开源代码。笔者在理解并掌握代码的基础上对代码进行了更新和调节。
>
> [examples/imagenet at main · pytorch/examples · GitHub](https://github.com/pytorch/examples/tree/main/imagenet)
>
> 以下中文部分是笔者扩展、修改、完善上述代码后的结果；英文部分README是官方教程。

# SML自建代码

笔者在官方开源代码的基础上添加了以下部分：

- 自行构建了一套分离式、模块化的训练框架。详情结构参见实验报告。
- 在开源代码的基础上加入了自动resume、输出训练日志（包括精度、loss值等）便于重复训练和观察实验结果。
- 在开源代码的基础上加入了自建Network模块，使得自定义的模型结构可以无障碍衔接到模型中。
- 自行构建了读取数据集并分成训练集、验证集、测试集，使得官方数据集和自建数据集都能够很好地衔接到训练代码中。
- 加入了cam.py模块进行CAM图的绘制，使用时需要改动以下三点：
...
其他部分具体使用参见main.py和main_sml.py的参数解析和下面英文版教程。
