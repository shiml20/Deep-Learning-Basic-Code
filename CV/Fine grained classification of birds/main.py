...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc1 = validate(val_loader, model, criterion, best_acc1, args)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            }, str(epoch), is_best, output_dir=args.output_dir)
...
def validate(val_loader, model, criterion, best_acc1, args):
    ...
    def run_validate(loader, base_progress=0):
        with torch.no_grad():
            ...
            for i, (images, target) in enumerate(loader):
                ...
                if i % args.print_freq == 0:
                    progress.display(i + 1, args.output_dir)
                    
                labels += target.cpu()
                predicted += _predicted.cpu()
    ...
    # sml
    best_acc = AverageMeter('MaxAcc@1', ':6.2f', Summary.AVERAGE)
    best_acc.update(best_acc1)
    top1 = AverageMeter('Acc@1', ':6.2f', Summary.AVERAGE)
    top5 = AverageMeter('Acc@5', ':6.2f', Summary.AVERAGE)
    progress = ProgressMeter(
        len(val_loader) + (args.distributed and (len(val_loader.sampler) * args.world_size < len(val_loader.dataset))),
        [batch_time, losses, best_acc, top1, top5],
        prefix='Test: ')
    ...
    progress.display_summary(args.output_dir)

    return top1.avg



def save_checkpoint(state, epoch_name, is_best, output_dir):
        
    checkpoint_paths = os.path.join(output_dir, f'checkpoint-{epoch_name}.pth')
    torch.save(state, checkpoint_paths)
    if is_best:
        checkpoint_paths_best = os.path.join(output_dir, f'checkpoint-best.pth')
        shutil.copyfile(checkpoint_paths, checkpoint_paths_best)
...
class ProgressMeter(object):
    ...
    def display(self, batch, output_dir):
    ...
    def display_summary(self, output_dir):
        entries = [" *"]
